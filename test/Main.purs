...
foreign import foldMap1NEArray
  :: forall r a. (r -> r -> r) -> (a -> r) -> NEArray a -> r
...
main = do
  ...
  assert $
    case
      findWithIndex (\i x -> i `mod` 2 == 0 && x `mod` 2 == 0) [ 1, 2, 4, 6 ]
      of
      Nothing -> false
  ...
  assert $ "*0a*1b*" == surroundMapWithIndex "*" (\i x -> show i <> x)
    [ "a", "b" ]
  assert $ "*0a*1b*2c*" == surroundMapWithIndex "*" (\i x -> show i <> x)
    [ "a", "b", "c" ]

  log "Test Foldable1 defaults"
  assert $ "(a(b(cd)))" == foldMap (foldr1 (\x y -> "(" <> x <> y <> ")"))
    (maybeMkNEArray [ "a", "b", "c", "d" ])
  assert $ "(((ab)c)d)" == foldMap (foldl1 (\x y -> "(" <> x <> y <> ")"))
    (maybeMkNEArray [ "a", "b", "c", "d" ])
...
testFoldableWithIndexFWith f n = do
  ...
  assert $ foldMapWithIndex (\i x -> Additive $ (i + 1) * x) dat == Additive
    expectedSum
...
testFoldableWithIndexLawsOn c f g = do
  ...
  assert $ foldMapWithIndex f c == foldlWithIndexDefault (\i y x -> y <> f i x)
    mempty
    c
  assert $ foldMapWithIndex f c == foldrWithIndexDefault (\i x y -> f i x <> y)
    mempty
    c
...
testTraversableFWith f n = do
  ...
  when (len <= 10) do
    result <- deferEff \_ -> traverse (\x -> [ x, x ]) dat == arrayReplicate
      (pow 2 len)
      dat
...
testTraversableWithIndexFWith f n = do
  ...
  assert $ traverseWithIndex (\i -> Just <<< Tuple i) dat == Just
    (mapWithIndex Tuple dat)
  assert $ traverseWithIndex (const Just) dat == traverse Just dat
  assert $ traverseWithIndex (\i -> pure <<< Tuple i) dat ==
    [ mapWithIndex Tuple dat ]
  assert $
...
instance eqFML :: (Eq a) => Eq (FoldMapDefaultL a) where
  eq (FML l) (FML r) = l == r

instance eqFMR :: (Eq a) => Eq (FoldMapDefaultR a) where
  eq (FMR l) (FMR r) = l == r

instance eqFLD :: (Eq a) => Eq (FoldlDefault a) where
  eq (FLD l) (FLD r) = l == r

instance eqFRD :: (Eq a) => Eq (FoldrDefault a) where
  eq (FRD l) (FRD r) = l == r
...
instance eqTD :: (Eq a) => Eq (TraverseDefault a) where
  eq (TD l) (TD r) = l == r

instance eqSD :: (Eq a) => Eq (SequenceDefault a) where
  eq (SD l) (SD r) = l == r

instance functorTD :: Functor TraverseDefault where
  map f (TD a) = TD (map f a)

instance functorSD :: Functor SequenceDefault where
  map f (SD a) = SD (map f a)
...
testBifoldableIOrWith lift fst snd u = do
  ...
  assert $ bifoldMap Additive Additive (lift $ Both fst snd) == Additive
    (fst + snd)
...
testBitraversableIOrWith lift = do
  ...
  assert $ bisequence (lift (Both (Just true) (Just false))) == just
    (Both true false)
  assert $ bisequence (lift (Fst (Just true))) == just
    (Fst true :: IOr Boolean Boolean)
  assert $ bisequence (lift (Snd (Just false))) == just
    (Snd false :: IOr Boolean Boolean)
  assert $ bitraverse Just Just (lift (Both true false)) == just
    (Both true false)
  assert $ bitraverse Just Just (lift (Fst true)) == just
    (Fst true :: IOr Boolean Boolean)
  assert $ bitraverse Just Just (lift (Snd false)) == just
    (Snd false :: IOr Boolean Boolean)
...
instance eqBFML :: (Eq l, Eq r) => Eq (BifoldMapDefaultL l r) where
  eq (BFML l) (BFML r) = l == r

instance eqBFMR :: (Eq l, Eq r) => Eq (BifoldMapDefaultR l r) where
  eq (BFMR l) (BFMR r) = l == r

instance eqBFLD :: (Eq l, Eq r) => Eq (BifoldlDefault l r) where
  eq (BFLD l) (BFLD r) = l == r

instance eqBFRD :: (Eq l, Eq r) => Eq (BifoldrDefault l r) where
  eq (BFRD l) (BFRD r) = l == r
...
instance eqBTD :: (Eq l, Eq r) => Eq (BitraverseDefault l r) where
  eq (BTD l) (BTD r) = l == r

instance eqBSD :: (Eq l, Eq r) => Eq (BisequenceDefault l r) where
  eq (BSD l) (BSD r) = l == r

instance bifunctorBTD :: Bifunctor BitraverseDefault where
  bimap f g (BTD m) = BTD (bimap f g m)

instance bifunctorBSD :: Bifunctor BisequenceDefault where
  bimap f g (BSD m) = BSD (bimap f g m)
